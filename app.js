...
function speakGuide(text, mood='talk'){const box=$('guideBubble'),img=$('guideImg');if(!box)return;box.classList.remove('show');if(img)img.classList.remove('bounce','wave');setTimeout(()=>{box.textContent=text;box.classList.add('show');if(img)img.classList.add(mood==='wave'?'wave':'bounce')},90)}
function hideGuide(){const g=$('guide');if(g)g.classList.remove('show')}
function showGuide(){const g=$('guide');if(g)g.classList.add('show')}
function openLogin(){ $('opening')?.classList.add('hidden'); $('loginScreen')?.classList.remove('hidden'); speakGuide('👋 Hai! Selamat datang di Aspiroom! Aku bakal nemenin kamu selama mengisi aspirasi.');}
...
async function registerStudent(){
 ...
 let loaded=false; for(let i=0;i<6;i++){try{await loadMe();loaded=true;break}catch(e){await sleep(500)}} if(!loaded)return setErr('Akun berhasil dibuat, tetapi profil siswa belum siap. Coba masuk lagi dengan username dan password yang baru dibuat.'); localStorage.setItem('aspiroom_tutorial_done',''); location.href='student.html';
}
...
function tutorial(){if(localStorage.getItem('aspiroom_tutorial_done')==='1')return;showGuide();speakGuide('👋 Hai! Selamat datang di Aspiroom! Aku bakal nemenin kamu selama mengisi aspirasi.');const next=()=>{localStorage.setItem('aspiroom_tutorial_done','1');hideGuide()};$('guideNext').onclick=next;$('guideSkip').onclick=next;}
